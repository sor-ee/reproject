...
let testcart = {
  ...
  resetStock: () => {
    for (let total in product) {
        const haveInCart = testcart.productcart.find((c) => c.id === product[total].productId)
        if (haveInCart) {
...
function addProductToCart() {
  for (let n in product) {
    ...
    addBt.addEventListener(
      ...
      () => {
        if (product[n].total !== 0) {
          const alreadyInCart =  testcart.productcart.find((c) => c.id === product[n].productId);
          if (alreadyInCart) {
